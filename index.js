...
function load(){
  try{
    ...
    if(fs.existsSync('./signals.json')){
      ...
      greens=g;reds=r;
      const t=g+r;
      if(t>0) assert=Math.round((g/t)*100);
    }
  }catch(e){}
}
...
function saveH(){
  try{
    fs.writeFileSync('./history.json',
      JSON.stringify(history.slice(0,400)));
  }catch(e){}
}
...
function saveS(){
  try{
    fs.writeFileSync('./signals.json',
      JSON.stringify(signals.slice(0,100)));
  }catch(e){}
}
  ...
  for(let c=0;c<cols.length;c++){
    ...
    for(let i=3;i<history.length-5;i++){
      ...
      if(cur && cur.winner==='TIE'){
        const bef=history[i+1];
        const corAntes=bef?bef.winner:null;
        if(corAntes && corAntes!=='TIE' && corAntes===color){
          total++;
          const aft=history[i-3];
          const corDepois=aft?aft.winner:null;
    ...
    if(total>=3){
      pats.push({cor:color,taxa:taxa,ac:voltou,tt:total});
    }
  }
  pats.sort(function(a,b){return b.taxa-a.taxa;});
  return pats;
}

function analyze(){
  if(history.length<20){
    aiState='AGUARDANDO '+history.length;
    return;
  }
  aiState='IA ESCANEANDO '+history.length;
  const pats=calcPatterns();
  if(currentSignal){
  ...
  if(!best || best.taxa<60){
    let txt='ANALISANDO';
    if(best) txt+=' Melhor: '+best.cor+' '+best.taxa.toFixed(0)+'%';
    else txt+=' nenhum';
    aiState=txt;
    return;
  }
  ...
  if(ultimo==='TIE'){
    ...
    if(corAntes && corAntes!=='TIE'){
      ...
      for(let i=0;i<pats.length;i++){
        if(pats[i].cor===corAntes){pat=pats[i];break;}
      }
      if(pat && pat.taxa>=60){
        currentSignal={
          id:Date.now(),
        ...
        copy.status='WAITING';
        copy.pattern=corAntes+' antes TIE volta pular 2 - ';
        copy.pattern+=pat.taxa.toFixed(0)+'% ('+pat.ac+'/'+pat.tt+')';
...
function checkSig(newWinner){
  ...
  const t=greens+reds;
  if(t>0) assert=Math.round((greens/t)*100);
}
...
function getHtml(){
  ...
  if(currentSignal){
    ...
    sinalHtml+='padding:12px;text-align:center">';
    sinalHtml+='<div style="font-size:10px;color:#6b7280">IA PULA 2</div>';
    sinalHtml+='<div style="font-size:11px;color:#9ca3af;margin:4px 0">';
    ...
    sinalHtml+='<div style="font-size:20px;font-weight:900;color:'+col+'">';
    sinalHtml+='ENTRA '+currentSignal.color+'</div>';
    sinalHtml+='<div style="font-size:11px;background:rgba(255,255,255,0.1);';
    sinalHtml+='display:inline-block;padding:4px 10px;border-radius:10px;margin-top:4px">';
    sinalHtml+='COBRIR EMPATE - '+currentSignal.time+' G'+currentSignal.gales+'</div>';
    sinalHtml+='<div style="font-size:10px;color:#10b981;margin-top:4px">';
    sinalHtml+='IA '+currentSignal.taxa.toFixed(0)+'% - G2 1-2-4</div></div>';
  }else{
    sinalHtml+='<div style="text-align:center;padding:16px">';
    sinalHtml+='<div style="font-size:24px">AI</div>';
    sinalHtml+='<div style="font-weight:800;margin-top:4px">AGUARDANDO</div>';
    sinalHtml+='<div style="font-size:10px;color:#6b7280;margin-top:2px">';
  ...
  html+='<div style="font-size:14px;font-weight:800;color:#ef4444">'+stats.HOME+' '+homePct+'%</div></div>';
  html+='<div class="glass" style="text-align:center">';
  html+='<div style="font-size:8px;color:#3b82f6">AWAY</div>';
  html+='<div style="font-size:14px;font-weight:800;color:#3b82f6">'+stats.AWAY+' '+awayPct+'%</div></div>';
  html+='<div class="glass" style="text-align:center">';
  html+='<div style="font-size:8px;color:#eab308">TIE</div>';
  html+='<div style="font-size:14px;font-weight:800;color:#eab308">'+stats.TIE+' '+tiePct+'%</div></div></div>';
  html+='<div class="glass" style="margin-bottom:8px">';
  ...
  html+='<div class="glass"><h3 style="font-size:9px;color:#6b7280;margin-bottom:6px">SINAIS G2</h3>';
  html+='<div>'+sinaisL+'</div></div>';
  html+='<div class="glass"><h3 style="font-size:9px;color:#6b7280;margin-bottom:6px">HISTORICO 400</h3>';
  html+='<table style="width:100%;font-size:10px"><thead><tr style="color:#6b7280;font-size:8px"><th>#</th><th>Res</th><th>Hora</th></tr></thead>';
  html+='<tbody>'+(linhas||'<tr><td colspan=3 style="text-align:center;padding:12px;color:#6b7280">Conectando...</td></tr>')+'</tbody></table></div></div>';
  html+='</div>';
  html+='<script>setInterval(()=>location.reload(),15000);</script>';
  html+='</body></html>';
  return html;
}
...
const server=http.createServer(async (req,res)=>{
  ...
  if(url.pathname==='/api/stats'){
    res.writeHead(200,{'Content-Type':'application/json'});
    const out={success:true,stats,total:history.length,botStatus,error:lastError,greens,reds,assert,aiState,signal:currentSignal,signals:signals.slice(0,20)};
    res.end(JSON.stringify(out));
    return;
  }
  if(url.pathname==='/api/clear'){
    try{fs.unlinkSync('./history.json');fs.unlinkSync('./signals.json');fs.unlinkSync('./token.json');}catch(e){}
    history=[];stats={HOME:0,AWAY:0,TIE:0};signals=[];currentSignal=null;greens=0;reds=0;assert=0;
    await fetchReal();
    res.writeHead(200,{'Content-Type':'application/json'});
  ...
  if(url.pathname==='/api/raw'){
    try{
      ...
      res.writeHead(200,{'Content-Type':'application/json'});
      res.end(JSON.stringify({count:data.length,distinct,sample:data.slice(0,3)},null,2));
